Fabric = class {
	...
	start() {
		...
		if (this.isTlsEnabled()) {
			var dockerFile = './docker-compose.tls.yml';
		}
		var spawn = require('child_process').spawnSync;
		return new Promise(function(resolve, reject) {
			var net = spawn('docker-compose', ['-f', dockerFile, 'up', '-d', '--build'], {
				encoding: 'utf8',
	...
	stop() {
		...
		var spawn = require('child_process').spawnSync;
		return new Promise(function(resolve, reject) {
			var net = spawn('docker-compose', ['-f', dockerFile, 'stop'], {
				encoding: 'utf8',
				stdio: ['ignore', 'pipe', 'pipe'],
				cwd: tutil.goPath + '/src/github.com/hyperledger/fabric-sdk-node/test/unit',
			});
			if (net.status != 0 && (typeof net.status != null)) {
				...
				var net = spawn('docker-compose', ['-f', dockerFile, 'rm', '-f'], {
					encoding: 'utf8',
					stdio: ['ignore', 'pipe', 'pipe'],
					cwd: tutil.goPath + '/src/github.com/hyperledger/fabric-sdk-node/test/unit',
				});
